function [H_position, C_position, T_position, posicionH, posicionT,pcdata,orient]=HCT_tracker(obj,start_tr,end_tr,bg_s,bg_e,visualization,st_pt,threshold,poly_med)
...
% Usage of mask
use_mask=2; % if you want to use mask, if yes then '1'(for home compartment and circular mask), '2'(only for circular mask) else '0',
...
for tt=end_tr-1:-1:start_tr
    ...
    if use_mask==1
        ...
        Use_I(c_mask==0)=0;% applying circular mask
    elseif use_mask==2
        Use_I(c_mask==0)=0;% applying circular mask
    end
    %%
    ...
    if blob_med==1
        ...
        % the fitted line to find the head and tail position.
